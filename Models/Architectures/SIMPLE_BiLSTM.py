...
class SIMPLE_BiLSTM:
    ...
    def create_bilstm(self):
        ...
        x = Bidirectional(LSTM(units=self.rec_units, activation="relu", return_sequences=True))(x)
        x = Bidirectional(LSTM(units=self.rec_units, activation="relu", return_sequences=False))(x)
